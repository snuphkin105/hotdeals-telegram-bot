...
async def run_search(
    ...
    for index, product in enumerate(products, start=1):
        ...
        link_note = (
            ...
            else "ℹ️ Direct product link. Affiliate conversion was unavailable for this item."
        )
        button_text = "🛒 Open affiliate product" if product.is_affiliate else "🛒 Open product"
        caption = (
            f"<b>{index}. {title}</b>\n"
        ...
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton(button_text, url=product.affiliate_url)]]
        )
        try:
